...
export default function ChatList() {
    ...
    // Note: API returns user objects with 'last_message_timestamp'
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-[#101322] text-[#101322] dark:text-white font-['Be_Vietnam_Pro'] pb-32 overflow-x-hidden transition-colors duration-500">
            <header className="sticky top-0 z-50 bg-white/90 dark:bg-[#101322]/90 backdrop-blur-xl px-6 py-4 border-b border-black/5 dark:border-white/5 transition-all">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <Link to="/settings" className="size-10 rounded-xl bg-gray-100 dark:bg-[#1a1f35] flex items-center justify-center border border-black/5 dark:border-white/10">
                        ...
                        <div className="p-4 space-y-1">
                            ...
                            <AnimatePresence>
                                {filteredConversations.map((match, idx) => (
                                    ...
                                    >
                                        <div className="absolute inset-y-0 right-0 w-20 bg-red-500 rounded-[2rem] flex items-center justify-center z-0 my-1">
                                        ...
                                        >
                                            <Link
                                                to={`/chat/${match.id}`}
                                                className={`flex items-center gap-4 p-4 rounded-[2rem] transition-all active:scale-[0.98] transition-colors duration-500 hover:bg-white dark:hover:bg-white/5 border border-transparent`}
                                            >
                            ...
                            {!loading && filteredConversations.length === 0 && activeConversations.length > 0 && (
                                <div className="py-10 text-center opacity-40">
                                    <p className="text-xs italic">Aucun résultat pour "{searchQuery}"</p>
                                </div>
                            )}

                            {!loading && activeConversations.length === 0 && newMatches.length === 0 && (
