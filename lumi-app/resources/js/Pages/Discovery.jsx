...
export default function Discovery() {
    ...
    const handleSwipe = async (status) => {
        ...
        try {
            ...
            if (data.is_mutual) {
                navigate(`/match/success/${target.id}`);
            }
        } catch (err) {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-gray-50 dark:bg-[#101322] flex flex-col items-center justify-center space-y-6">
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-[#101322] font-['Be_Vietnam_Pro'] text-[#101322] dark:text-white transition-colors duration-500 flex flex-col relative overflow-hidden">
            ...
            <header className="flex items-center bg-white dark:bg-[#101322] p-4 pb-2 justify-between sticky top-0 z-20 border-b border-black/5 dark:border-white/10 transition-colors duration-500">
                <div className="flex size-12 items-center justify-start">
                    {/* Settings/Profile */}
                    <button onClick={() => navigate('/settings')} className="flex cursor-pointer items-center justify-center rounded-full h-12 bg-transparent text-[#101322] dark:text-white hover:text-[#D4AF37] transition-colors">
                ...
                <div className="flex w-12 items-center justify-end">
                    <button onClick={() => navigate('/settings/notifications')} className="flex cursor-pointer items-center justify-center rounded-full h-12 bg-transparent text-[#101322] dark:text-white hover:text-[#D4AF37] transition-colors">
            ...
            <main className="flex-1 relative p-4 flex flex-col items-center justify-center mb-24">
                ...
                {profiles.length > currentIndex && (
                    <div className="flex w-full justify-center gap-6 pt-8 items-center">
                        ...
                        <button
                            onClick={() => navigate(`/chat/${profiles[currentIndex].id}`)}
