...
export default function PhotoManagement({ photos: initialPhotos }) {
    ...
    const handleFileUpload = async (e) => {
        ...
        reader.onloadend = async () => {
            try {
                await axios.post(route('photos.add'), { photo: reader.result });
                // We refresh through Inertia to get the updated list
                router.reload({ only: ['photos'], onSuccess: (page) => setPhotos(page.props.photos) });
            } catch (err) {
                console.error(err);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(route('photos.delete', id));
    ...
    const handleReorder = async (newPhotos) => {
        setPhotos(newPhotos);
        try {
            await axios.post(route('photos.reorder'), {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-[#101322] text-[#101322] dark:text-white font-sans pb-32 transition-colors duration-500">
            <Head title="Gérer mes photos - Lumi" />
            ...
            <header className="sticky top-0 z-50 px-6 py-4 flex items-center justify-between border-b bg-white/90 dark:bg-[#101322]/90 border-black/5 dark:border-white/10 backdrop-blur-xl transition-all">
                <Link href={route('profile.edit')} className="w-10 h-10 flex items-center justify-start text-[#101322] dark:text-white">
